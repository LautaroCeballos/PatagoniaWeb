# Plan Maestro Unificado v3.1 — SART3 Booking Portal

Fecha: 2026-05-25 (v3.1 — Paddle reemplazado por Mercado Pago, marketplace OAuth multi-inquilino)
...
lib/
├── actions/
│   ├── public.ts                  ← searchAvailableRooms() + createPublicBooking() (con Zod)
│   ├── public-payment.ts          ← createMercadoPagoPreference() (usando token del seller via OAuth)
...
│   └── _validation.ts             ← Zod validation wrapper
...
├── definitions.ts                 ← Payment.method (+ 'MercadoPago'), PropertySettings con mp tokens
...
app/[locale]/book/[slug]/
  (colores hardcodeados slate convertidos a theme-aware)

messages/{es,en,pt}.json           ← + namespace "Booking" + "Settings.theme" + "Settings.mercadopago"

next.config.ts                     ← CSP actualizado (MP domains: mercadopago.com, mercadolibre.com)
...
Reemplazar colores `slate-*` hardcodeados por CSS variables o clases theme-aware:
...
### 1.4 Unificar PropertySettings + resolver naming collision + MP tokens
...
### 3.3 createMercadoPagoPreference (dinámico + Zod + token del seller)
...
import { createAdminClient } from "@/lib/server/appwrite"
import { validateOrThrow } from "./_validation"
import { z } from "zod"
...
### 3.8 Modificar createPublicBooking
...
1. Guest completa GuestInfoForm → Zod validation → createPublicBooking()
   (userId se lee de roomDoc.userId, NO del input del cliente)
...
### 4.4 Rate Limiting
- In-memory Map<IP, count> TTL 60s, 10/min/IP. Documentar deuda técnica para serverless.

### 4.5 Zod Validation
- Todas las server actions públicas: `searchAvailableRooms`, `createPublicBooking`, `createMercadoPagoPreference`
...
# Eliminar dominios Paddle (booking), conservar para suscripciones si aplica
script-src 'self' 'unsafe-eval' 'unsafe-inline'
style-src 'self' 'unsafe-inline'
img-src 'self' data: blob: https://cloud.appwrite.io https://http2.mlstatic.com https://*.mercadopago.com
...
⚠️ **Nota**: Checkout Pro es redirección completa (no overlay), por lo que NO necesita `script-src` ni `frame-src` para los dominios de pago. Solo `form-action` para POST redirect e `img-src` para imágenes de productos. `*.mercadopago.com` cubre ccTLDs: `.com.ar`, `.com.br`, `.cl`, `.co`, `.com.mx`, etc.
...
### 5.1 JSON-LD
- `Hotel` + `BreadcrumbList` + `FAQPage` (opcional)
- Hotel con `@id`, `telephone`, `address`, `priceRange`

### 5.2 Hreflang Fix (completo — 8 archivos)
...
**Archivo**: `app/[locale]/layout.tsx` **[MODIFICAR]** — cambiar hreflang de `es-ES`/`en-US`/`pt-BR` a `es`/`en`/`pt`

**Archivo**: `app/[locale]/book/[slug]/layout.tsx` **[MODIFICAR]** — usar locale real de `params`:
```typescript
const { locale, slug } = await params
// ⇒ getHreflang(locale, `/book/${slug}`)
```

**Archivos (cambian automáticamente)**: `page.tsx`, `signin/page.tsx`, `signup/page.tsx`, `pricing/page.tsx`, `terms/page.tsx`, `privacy/page.tsx`, `refund/page.tsx` — todos usan `getHreflang(locale, pathname)`.

### 5.3 Meta Tags
- OG image dinámica, OpenGraph locale real, Twitter card, canonical dinámico
- `noindex, follow` en results / `noindex, nofollow` en checkout+confirmation

### 5.4 Sitemap
- `getAllPropertySlugs()` extendido para retornar `updatedAt`
...
## Fase 6: Personalización Visual desde Settings

### 6.1 Data Layer
- `PropertySettings` ya unificada en `definitions.ts` (Fase 1.4)
- `updatePropertySettings()` extendido en `property.ts`
- `getPropertyBySlug()` devuelve theme fields (tipado desde Fase 1.4)

### 6.2 Theme Presets (6 presets)
...
### 6.3 Settings UI
- Presets selector (6 círculos), color pickers, font selector, image upload, live preview
- "Guardar" y "Restaurar default" con confirm dialog
- Sección "Conectar Mercado Pago": botón connect/disconnect, estado de conexión

### 6.4 Theme Engine
- `BookingThemeProvider` (Fase 0.4) conectado con datos de prefs en Fase 6
- CSS variables via `:root:has(.booking-theme[data-preset="..."])`

---

## Resumen de Archivos por Fase (v3.1)
...
| **4** | — | `webhooks/mercadopago/route.ts`, `next.config.ts` (CSP), `public.ts`, `public-payment.ts`, `_validation.ts` |
| **5** | — | `seo.ts`, `layout.tsx` (root + booking), `sitemap.ts`, `property.ts`, `page.tsx`, `signin/page.tsx`, `signup/page.tsx`, `pricing/page.tsx`, `terms/page.tsx`, `privacy/page.tsx`, `refund/page.tsx` |
| **6** | — | `definitions.ts`, `property.ts`, `settings/page.tsx`, `layout.tsx`, `.env.example`, `next.config.ts` |
...
## Orden de Implementación Sugerido (v3.1)
...
Fase 1 (fundación):
  ...
  ├── PropertySettings + mp tokens en definitions.ts
...
Fase 3 (pagos — Mercado Pago):
  ...
  ├── lib/actions/public.ts — createPublicBooking mejorado (userId de DB)
  ├── app/api/webhooks/paddle/route.ts — limpiar eventos booking
  └── Settings: sección "Conectar Mercado Pago"

Fase 4 + 5 + 6 (seguridad + SEO + customization):
  ├── CSP completo (dominios MP en form-action + img-src)
  ├── Rate limiting + Zod en server actions
  ├── Hreflang fix completo (seo.ts + root layout + booking + 6 pages)
  ├── JSON-LD + OG images + sitemap
  ├── Settings UI (presets + color pickers + preview)
  └── Theme engine conectado con datos guardados
```

---

## Checklist de Validación Post-Implementación (v3.1)
